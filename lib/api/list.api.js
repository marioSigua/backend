...
router.get('/student/question/:token', async (req, res, next) => {
...
router.get('/exam/question/:subject_code', async (req, res, next) => {
     const { subject_code } = req.params
     try {
          const questions = await SubjectModel.query()
               .withGraphJoined('questionTopics')
               .where('subjects_tbl.subject_code', subject_code)
