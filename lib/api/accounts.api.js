...
router.post('/create/form/questions', async (req, res, next) => {
    const { term, question_form, subject_code, stdEmail } = req.body,
        formData = {
            term,
            question_form,
            subject_code,
    ...
    try {
        const questData = await QuestionModel.query().insert(formData)

        if (questData) {
            let url = await jwt.sign(questData.question_id)
            mailOptions.html = `<h1>hahahahah ${url} tangaka</h1>`
            stdEmail.forEach(function (to, i, array) {
                mailOptions.to = to
