...
router.post('/accounts/login', async (req, res, next) => {
     ...
     try {
          const isVerified = await AccountModel.query().findOne({
               email: email,
          })

          if (isVerified.isActive === 0)
...
router.post('/accounts/signup', async (req, res, next) => {
     ...
     try {
          ...
          admin.html = ` <p>Fullname of Account Owner: ${firstname} ${lastname}</p>
                         ...
                         <p>This is the batch number ${d} of ${email} that he/she will present the same batch number that we sent on this email <br /> <a href="http://localhost:8080/account/verification/${urlToken}">Click here to Verify Account</a> </p>
...
router.post('/create/form/questions', cpUpload, async (req, res, next) => {
     const { batch_number, term, question_form, subject_code, stdEmail } =
          req.body
