...
const SubjectReference = require("../../database/models/subjreference.model");
...
router.patch("/students/dropall", async (req, res, next) => {
  const { account_id, academic_year, past_year } = req.body;
  try {
    // EnrolledStudents.query().where('')
    console.log(req.body);

    const subjects = await SubjectModel.query()
      .withGraphJoined("SubjectsGrade")
      .where("account_id", account_id)
      .andWhere("current_year", past_year)
      .delete();

    await AccountModel.query()
      .where("account_id", account_id)
      .patch({ academic_year: academic_year });
...
router.post("/accounts/signup", async (req, res, next) => {
  ...
  try {
    ...
    const d = Date.now();
    const adminEmail = "mario1313aa@gmail.com";
    ...
    let sendList = [admin, student];
    await Promise.all(
      sendList.map(async (k) => {
        let sent = await sendMail(k);
        console.log(sent);
      })
    );
...
router.patch("/verify/account", async (req, res, next) => {
  ...
  try {
    ...
    if (isVerified.isActive === 1) {
      ...
      const student = await AccountModel.query().findById(decoded.id).patch({
        isActive: 1,
      });
...
router.post("/create/form/questions", cpUpload, async (req, res, next) => {
  ...
  try {
    const questData = await QuestionModel.query().insertGraph(dispatchData);
...
router.post("/add/prof/subjects", async (req, res, next) => {
  const {
    ref_id,
    subject_course,
    section,
  ...
  try {
    ...
    const isExist = await SubjectModel.query().findOne({
      subject_sem: subject_sem,
      subject_ref: `${ref_id}-${section}`,
      isOccupied: 1,
      account_id: decoded.id,
    });

    if (isExist) throw new Error("Section and Subject is Currently Active");

    const isSuccess = await SubjectModel.query().insert({
      account_id: decoded.id,
      subject_code: `${ref_id}-${section}-${d}`,
      isOccupied: 1,
      subject_course,
      subject_ref: `${ref_id}-${section}`,
      current_year,
...
router.post("/add/students", async (req, res, next) => {
  ...
  try {
    ...
    if (subjectExist) throw new Error(`Student is already added on the class`);
...
router.post("/add/old/students", async (req, res, next) => {
  ...
  try {
    ...
    const students = await StudentModel.query().whereIn("student_id", list);
    ...
    if (checkIfDrop.length > 0 && payload.length > 0) {
      ...
      await EnrolledStudents.query().whereIn("student_id", ids).update({
        isDropped: 0,
      });

      const enrollStudent = await EnrolledStudents.query().insertGraph(payload);
...
router.post("/student/response", async (req, res, next) => {
  ...
  try {
    ...
    const isSuccess = await ResponseModel.query().insertGraph(studentResponse);
...
router.patch("/essay/score", async (req, res, next) => {
  ...
  try {
    ...
    let data = isFinished.map((k) => {
      let foundData = formList.find((el) => el.form_number === k.form_number);
...
router.post("/add/subject/reference", async (req, res, next) => {
  const { subject_name, ref_id, subject_course, subject_year } = req.body;

  try {
    const ifExist = await SubjectReference.query().findOne({ ref_id: ref_id });

    if (ifExist)
      throw new Error(
        `Subject Code: ${ifExist.ref_id}  Subject Name: ${ifExist.subject_name}  Already Added`
      );

    await SubjectReference.query().insert(req.body);
...
router.patch("/edit/subject/reference", async (req, res, next) => {
  const { subject_name, ref_id, subject_course, subject_year } = req.body;
  ...
  try {
    ...
    if (!ifExist)
      throw new Error(
        `Subject Code: ${ref_id}  Subject Name: ${subject_name}  Does not Exist`
      );

    await SubjectReference.query().where("ref_id", ref_id).patch(req.body);
